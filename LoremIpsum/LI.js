...
const paragraphNum = document.getElementById("p_num")
const form = document.querySelector(".form")
const dataSection = document.querySelector(".content")

form.addEventListener("submit", (e) => {
    // by default form sends data to server, since, we want to access that data here and are not sending data to server, hence preventing Defaut.
    e.preventDefault()
    dataSection.innerHTML = ""
    let num = parseInt(paragraphNum.value)
    ...
    let selection = data.slice(0, num)
    selection = selection.map((item) => {
        return `<p>${item}</p>`
    }).join("")

    dataSection.innerHTML = selection
})
